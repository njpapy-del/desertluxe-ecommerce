...
export async function generateMetadata(
  ...
  if (!product) return { title: 'Produit introuvable | My Luxury' }

  const url = `${BASE_URL}/product/${product.slug}`
  const img = product.images[0]

  return {
    title: `${product.name} — ${product.price}€ | My Luxury`,
    description: product.description.slice(0, 155),
    alternates: { canonical: url },
    openGraph: {
      type:        'website',
      url,
      title:       `${product.name} | My Luxury`,
      description: product.description.slice(0, 155),
...
export default function ProductPage({ params }: { params: { id: string } }) {
  ...
  const jsonLd = {
    ...
    sku:         product.id,
    brand: { '@type': 'Brand', name: 'My Luxury' },
    offers: {
      '@type':       'Offer',
      ...
      seller: { '@type': 'Organization', name: 'My Luxury' },
    },
