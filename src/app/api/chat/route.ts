...
function buildSystemPrompt(): string {
  return `Tu es Leila, la conseillère IA de MA LUXURY — une marque de mode luxe inspirée de Dubaï.
...
function ruleBasedResponse(message: string): string {
  ...
  if (lower.includes('best') || lower.includes('populaire') || lower.includes('recommande')) {
    ...
    return `Nos pièces phares du moment : ${best}. Des créations incontournables de la collection MA LUXURY !`
  }
  ...
  if (lower.includes('bonjour') || lower.includes('hello') || lower.includes('salut') || lower.includes('hi')) {
    return 'Bonjour ! Bienvenue chez MA LUXURY. Je suis Leila, votre conseillère. Comment puis-je vous aider à trouver la pièce parfaite ?'
  }
